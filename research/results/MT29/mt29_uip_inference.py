    ...
    def _lazy(self):
        if self._ase_calc is None:
            ...
            try:
                ...
                if 'modal' not in str(e):
                    raise
                # sevenn >= 0.13 requires selecting a fidelity head for
                # multi-fidelity checkpoints (observed on-box 2026-07-10:
                # "modal argument missing (avail: ['omat24', 'mpa'])").
                # Default 'mpa' = the MP-aligned head, matching this paper's
                # MP reference energies / WBM hull frame; override via
                # SEVENN_MODAL for sensitivity checks. Disclosed in output.
                modal = os.environ.get('SEVENN_MODAL', 'mpa')
                print(f'sevennet: retrying with modal={modal!r} (multi-fidelity '
                      f'checkpoint; MP-aligned head is this paper\'s frame)')
                self._ase_calc = SevenNetCalculator(modal=modal, **kwargs)
        return self._ase_calc
...
class _OrbCalculator:
    ...
    def _lazy(self):
        if self._ase_calc is None:
            ...
            try:
                # layout through orb-models <= 0.6
                from orb_models.forcefield.calculator import ORBCalculator
            except ImportError:
                try:
                    # orb-models 0.7.x moved the ASE calculator here
                    # (verified on-box against 0.7.0, 2026-07-10)
                    from orb_models.forcefield.inference.calculator import ORBCalculator
                except ImportError as e:
                    raise RuntimeError(
                        'orb-models is installed but no ORBCalculator was found at '
                        'either known location (forcefield.calculator or '
                        'forcefield.inference.calculator) — probe the installed '
                        'version\'s layout before rerunning.') from e
            device = os.environ.get('ORB_DEVICE')
            if not device:
                try:
                    import torch
            ...
            orbff = pretrained.orb_v3_conservative_inf_omat(device=device, **kwargs)
            if isinstance(orbff, tuple):
                # orb-models >= 0.7: pretrained loaders return
                # (model, atoms_adapter), and the inference ORBCalculator
                # requires the adapter as its second positional argument.
                # Verified LIVE on-box against 0.7.0 (2026-07-10):
                # ORB_PROBE_OK on a 2-atom NaCl rocksalt smoke. The <=0.6
                # single-return path below is unchanged (0.5.5-verified).
                orb_model, orb_atoms_adapter = orbff
                self._ase_calc = ORBCalculator(orb_model, orb_atoms_adapter, device=device)
            else:
                self._ase_calc = ORBCalculator(orbff, device=device)
        return self._ase_calc
...
def _load_wbm_from_init_structs_json(path: str, limit: Optional[int] = None) -> Iterable[Structure]:
    """Parse the `2022-10-19-wbm-init-structs.json.bz2` DataFile (pandas
    column-orient: {material_id: {row: id}, formula_from_cse: {...},
    initial_structure: {row: pymatgen-Structure-dict}}). This is the SAME WBM
    initial-structure set as the extxyz DataFile in a different container
    format (verified complete on-box 2026-07-09: 256,963 rows) — used as a
    DISCLOSED delivery-format substitution because both figshare routes for
    the extxyz 403 from the box (ndownloader AND api.figshare.com). Parsed
    with plain json (no pymatgen import): symbols from
    sites[].species[0].element (WBM is ordered, occu=1), cartesian positions
    from sites[].xyz, cell from lattice.matrix."""
    import bz2
    opener = bz2.open if path.endswith('.bz2') else open
    with opener(path, 'rt') as fh:
        raw = json.load(fh)
    ids = raw['material_id']
    structs = raw['initial_structure']
    for i, row_key in enumerate(ids):
        if limit is not None and i >= limit:
            break
        sd = structs[row_key]
        yield Structure(material_id=str(ids[row_key]),
                        symbols=[site['species'][0]['element'] for site in sd['sites']],
                        positions=[list(site['xyz']) for site in sd['sites']],
                        cell=[list(v) for v in sd['lattice']['matrix']],
                        pbc=(True, True, True))


def load_wbm_structures(wbm_root: str, limit: Optional[int] = None) -> Iterable[Structure]:
    """Reads the WBM initial structures: prefers the extxyz DataFile
    (`*wbm-initial-atoms*`, ase-parsed), falls back to the
    `*wbm-init-structs*.json*` DataFile (json-parsed, see
    :func:`_load_wbm_from_init_structs_json`). NOTE (DATA_MANIFEST.md): a
    0-BYTE aborted extxyz download may exist — files under 1KB are ignored
    rather than trusted."""
    def _real(paths):
        return [p for p in paths if os.path.getsize(p) > 1024]

    extxyz = _real(sorted(glob.glob(os.path.join(wbm_root, '*wbm-initial-atoms*'))))
    if extxyz:
        try:
            from ase.io import iread
        except ImportError as e:
            raise RuntimeError(
                'ase is required for extxyz WBM structure loading; pip install ase '
                '(see RUNME_CONTAINER.md).') from e
        for i, atoms in enumerate(iread(extxyz[0])):
            if limit is not None and i >= limit:
                break
        ...
        return
    json_candidates = _real(sorted(glob.glob(os.path.join(wbm_root, '*wbm-init-structs*.json*'))))
    if json_candidates:
        yield from _load_wbm_from_init_structs_json(json_candidates[0], limit)
        return
    raise FileNotFoundError(
        f'no WBM initial-structure file (extxyz or init-structs json) found under '
        f'{wbm_root!r}; run fetch_data.sh first.')
