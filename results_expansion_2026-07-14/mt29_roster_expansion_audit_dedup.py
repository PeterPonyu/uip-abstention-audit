#!/usr/bin/env python
"""MT29 roster-expansion audit — RED-TEAM-ROUND2 remediation rerun (2026-07-14).

Identical to `research/results/MT29/mt29_roster_expansion_audit.py` (same frozen machinery,
same seeds, same estimand, same inclusion rule) EXCEPT it DROPS the duplicate roster member
`orb_v2_omat` from the analysis.

WHY orb_v2_omat is dropped (verified 2026-07-14, executor + Figshare-API refetch):
  * `~/mt_uip_roster25/orb_v2_omat_pred.csv` is the genuine published Figshare file 51607307
    ("orbff-v2-20241011.csv.gz", supplied_md5 == computed_md5 == d00f381dac27d367133452a24f17a4e5,
    re-downloaded 2026-07-14 and md5-gated) — it is NOT a corrupt/wrong file.
  * BUT that published Orb-v2 checkpoint is BYTE-IDENTICAL (max |Δe_form| = 0 over 256,963 rows)
    to the frozen headline anchor `~/mt_uip/orb_pred.csv` (the paper's 'orb' anchor). The frozen
    'orb' anchor — labelled "Orb-v2 MPtrj-only" in the roster fetch note — is in fact this general
    Orb-v2 (20241011) checkpoint, NOT the MPtrj-only file (Figshare 51607310,
    "orbff-mptrj-only-v2-20241014.csv.gz", which differs by mean|Δ| 0.0372 / max 1.994).
  * Consequently `orb_v2_omat` in the OMat arm is the SAME predictions already present as the
    frozen 'orb' anchor. It is not an independent OMat-generation UIP, so counting it double-
    counts one checkpoint under two generation labels. It is excluded from the generational
    contrast. All 44 roster raw sources pass the md5 gate (see verify_roster_md5.py 44/44);
    orb_v2_omat is the only non-independent member.

CPU/pandas/NumPy only. Reads converted preds from ~/mt_uip_roster25/. No GPU/download here.
"""
...
# --- red-team-round2 remediation: drop the non-independent duplicate ---
EXCLUDE = {"orb_v2_omat"}
EXCLUDE_REASON = {
    "orb_v2_omat": ("byte-identical (max|Δe_form|=0 over 256,963 rows) to the frozen 'orb' "
                    "headline anchor = published Orb-v2 Figshare 51607307 (orbff-v2-20241011); "
                    "not an independent OMat-generation UIP")
}
...
def main():
    ...
    base = load_base()
    excluded = []
    print(f"WBM base rows {len(base)}; roster models {len(models_meta)}; EXCLUDE {sorted(EXCLUDE)}", flush=True)

    per_model = {}
    for short in sorted(models_meta):
        if short in EXCLUDE:
            excluded.append(dict(short=short, generation=models_meta[short]["generation"],
                                 klass=models_meta[short]["klass"], reason=EXCLUDE_REASON.get(short, "")))
            print(f"  DROP {short}: {EXCLUDE_REASON.get(short,'excluded')}", flush=True)
            continue
    ...
    result = dict(
        analysis="MT29 roster-expansion generational contrast of oxide-anchored value-of-abstention "
                 "(RED-TEAM-ROUND2 remediation rerun 2026-07-14: duplicate orb_v2_omat dropped)",
        prereg="research/results/MT29/mt29_roster_expansion_PREREG-2026-07-13.md",
        remediation=dict(
            supersedes="results_expansion_2026-07-13/mt29_roster_expansion_result.json",
            excluded_models=excluded,
            note="orb_v2_omat excluded as a non-independent duplicate of the frozen 'orb' headline "
                 "anchor (both = published Orb-v2 Figshare 51607307, md5 d00f381...). All 44 roster "
                 "raw sources pass the md5 gate; this is the only non-independent member."),
    ...
    print(f"  oxide-halide subfamily ({mult_oxhal['n']} cells): CI-excl0 {mult_oxhal['n_ci_excl0']}, "
          f"BH {mult_oxhal['n_bh_q05']}, Holm {mult_oxhal['n_holm_a05']}", flush=True)
