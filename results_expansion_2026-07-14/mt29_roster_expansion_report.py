#!/usr/bin/env python
"""Render results_expansion_2026-07-14/RESULTS-ROSTER-EXPANSION.md from the audit JSON.
Numbers are read from mt29_roster_expansion_result.json — never hand-typed.
RED-TEAM-ROUND2 remediation rerun: orb_v2_omat dropped as a non-independent duplicate of the
frozen 'orb' anchor. Supersedes results_expansion_2026-07-13/ (see that dir's
CONTAMINATED-2026-07-14.md)."""
import json, os
...
L.append("")
L.append("**RED-TEAM-ROUND2 remediation rerun (2026-07-14).** Supersedes "
         "`results_expansion_2026-07-13/` (contaminated — see that dir's `CONTAMINATED-2026-07-14.md`). "
         "`orb_v2_omat` is dropped: it is byte-identical to the frozen 'orb' headline anchor (both are the "
         "published Orb-v2 Figshare file 51607307), so it is a non-independent duplicate, not an additional "
         "OMat-generation model. All 44 roster raw sources pass the md5 gate "
         "(`verify_roster_md5.py`, 44/44 OK); this is the only non-independent member, leaving 43 models "
         "converted / 32 included UIPs.")
L.append("")
L.append("**Status:** post-hoc-motivated, pre-specified analysis plan "
         "(`mt29_roster_expansion_PREREG-2026-07-13.md`, dated 2026-07-13; not git-timestamped ahead of the "
         "run, unlike the alt-stratification prereg). Frozen legacy-4 regression guard passed byte-exact "
         "before this run.")
...
L.append("## Roster")
L.append("")
L.append(f"- Roster source files fetched and md5-verified: 44. Independent models analyzed: "
         f"{R['n_models_total']} (all MP-gen: {gc['all_MP']}, all OMat-gen: {gc['all_OMat']}) "
         f"— `orb_v2_omat` excluded as a non-independent duplicate of the frozen 'orb' anchor "
         f"(see remediation note above).")
