...
out_stem <- "figures/F2_interaction_heatmap"
...
lim <- max(abs(ints$interaction_med))
...
p <- ggplot(ints, aes(x = model_lab, y = pair, fill = interaction_med)) +
  ...
  geom_text(aes(label = sprintf("%+.2f", interaction_med)), size = 3.0) +
  scale_fill_gradient2(
    low = "#2166AC", mid = "white", high = "#B2182B",
    midpoint = 0, limits = c(-lim, lim),
    name = expression("interaction " * gain[A] - gain[B] * " (" * Delta * "DAF, median)")
  ) +
  labs(x = NULL, y = NULL) +
...
# the 7pt floor).
ggsave(paste0(out_stem, ".pdf"), p, width = 6.27, height = 4.88, device = cairo_pdf, family = ff)
ggsave(paste0(out_stem, ".png"), p, width = 6.27, height = 4.88, dpi = 300,
       device = ragg::agg_png)
