...
p <- ggplot(df, aes(x = abs_med, fill = kind)) +
  geom_histogram(aes(y = after_stat(density)), bins = 16, alpha = 0.75,
                 position = "identity", colour = "grey20", linewidth = 0.5) +
  geom_label(
    data = ann, aes(x = Inf, y = Inf, label = label),
    # size ~1.7 ≈ 4.8pt on this canvas (~6pt after full-width include).
    hjust = 1.02, vjust = 1.08, size = 1.7,
    fill = "white", colour = "grey15", linewidth = 0.18,
    label.padding = unit(0.06, "lines"), label.r = unit(0.05, "lines"),
    inherit.aes = FALSE
  ) +
...
# Taller canvas keeps facet annotations clear at manuscript width.
ggsave(paste0(out_stem, ".pdf"), p, width = 6.27, height = 2.75, device = cairo_pdf, family = ff)
ggsave(paste0(out_stem, ".png"), p, width = 6.27, height = 2.75, dpi = 300,
       device = ragg::agg_png)
