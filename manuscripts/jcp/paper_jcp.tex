...
% manuscript ../paper.tex. ../paper.tex remains the CANONICAL SOURCE (content is fixed,
% red-team-hardened, and honest); this file is the JCP submission-format artifact only.
% Do NOT make scientific/content changes here without mirroring them in ../paper.tex.
% Every number in this file is verified identical to ../paper.tex (and to ../digital_discovery/
% paper_dd.tex, whose body this file re-templates without content changes).
%
% Retargeted 2026-07-16 from ../digital_discovery/paper_dd.tex per the user-ratified venue
% switch (DD gold-OA APC of GBP 2,200 exceeds the user's careful-adopt threshold; JCP's
% subscription route carries $0 publication charges and has direct in-venue precedent for
% UQ/reliability work on ML interatomic potentials -- see VENUE-MEMO-JCP-2026-07-16.md).
% ../digital_discovery/ is kept intact as a fallback kit, not deleted.
%
% CLASS NOTE: Live-verified 2026-07-16 against AIP Publishing's author instructions
% (publishing.aip.org/resources/researchers/author-instructions/) and the CTAN "Author's
% Guide to AIP Substyles for REVTEX 4.2" (aipguide4-2.pdf): AIP accepts author-prepared
% Word or LaTeX/REVTeX manuscripts; format-free initial submission is NOT offered (a single
% compiled manuscript PDF with all figures is required at first submission; a separate
% Cover Letter file is optional at that stage). REVTeX 4.2 with the aip/jcp substyle
% (\documentclass[aip,jcp,...]{revtex4-2}) is AIP's own LaTeX house style and is used here.
...
% shared.bib + refs.bib, unchanged from the Digital Discovery kit.
...
% ---- Title page ----
% JCP reframe (content-preserving, carried over unchanged from the Digital Discovery
% port): the title foregrounds the contribution as a reliability/evaluation methodology
% for universal MLIPs (a chemistry-stratified audit of selective abstention), which is
% also squarely in JCP's scope (see in-venue precedent, VENUE-MEMO-JCP-2026-07-16.md).
% R3 LEVER 2026-07-16 (FINAL-SCORE-R3-2026-07-16.md, task item (c)#3 / Reviewer 1 #2):
% "tracks reference-data trustworthiness" read as a causal locator claim the paper itself
% declines to establish; softened to co-location strength ("concentrates where ... is
% lowest"), matching the paper's own "we do not claim to have isolated a single physical
% cause" disclosure. No claim or number changed, presentation only.
% PHYSICS-LEAD TOPPER 2026-07-17 (DESK-CHECK-JCP §4.1): one verified physical-anchor sentence
%   prepended (RMSE ~7x line 681, GGA/Hubbard-U/MP2020 seam lines 143-145); effect-size-first
%   structure of the R4 lever below preserved unchanged.
% R4 LEVER 2026-07-17 (FINAL-SCORE-R4-2026-07-17.md, single highest-leverage remaining
% lever / Reviewer 1 #1, Reviewer 2 #1): the title asserted the trustworthiness
% *interpretation* as the finding, while SS IV(c) states the disagreement metric "cannot
% separate" UIP-fitting error from label unreliability -- a claim the paper itself
% certifies as unidentifiable does not belong in the title. Retitled to the demonstrated,
% control-hardened result (oxide-anchored, 32-UIP-universal abstention-value interaction);
% all trustworthiness/causal language removed from the title. No claim or number changed,
% presentation only.
...
% TODO-USER: confirm author name/ORCID/email; supply the affiliation (department,
% institution, city, postcode, country). Affiliation was never present in the canonical
% ../paper.tex and must be added before submission.
\author{Zeyu Fu}
...
\begin{abstract}
% Digital Discovery abstract, <=200 words (word count verified 2026-07-16 post-lever reframe;
% see verify step). Reframed to lead with the reference-data-trustworthiness thesis and promote
% the precision-point decomposition; every claim and number traces to the canonical ../paper.tex.
...
% pre-pass abstract (see paper_dd.tex.bak-pre-readerpass), only the ordering changed.
% R4 LEVER 2026-07-17 (FINAL-SCORE-R4-2026-07-17.md, single highest-leverage remaining lever
% / Reviewer 1 #1, #2, Reviewer 2 #1): re-led on the demonstrated, control-hardened
% precision-point interaction (matches SS III(C)'s own primary-effect-size recommendation);
% the trustworthiness reading is now a single, explicitly-flagged interpretive sentence
% ("cannot isolate ... from ... difficulty"), matching SS IV(c)'s "cannot separate them"
% standard. No claim or number changed, presentation and ordering only; word count re-verified
% (200 words).
...
they lack, and the layered MP2020 corrections are least mutually consistent. The value of
selective abstention in UIPs is therefore chemistry-dependent and, in our central finding,
oxide-anchored: a genuine $+7.7$
precision-point advantage (CHGNet oxide-vs-halide) that we establish with a reusable, CPU-only
matched-yield audit protocol, runnable on any published prediction file. Across six anion-class
strata and four UIPs on Matbench Discovery, the stratum-by-coverage interaction in discovery
acceleration factor (DAF) excludes zero in 45 of 60 cells under a prototype-blocked bootstrap
(48/60 i.i.d.), is oxide-anchored, and survives leave-one-element-out, WBM-round, multiplicity,
and alternative-stratification controls with a shuffle null. Low-base-rate normalisation
...
% source: results_expansion_2026-07-14/mt29_roster_expansion_result.json (n_included_uip 32; positive-anchored 32/32; V3 perm_p 0.68153; orb_v3 ox-hal 0.6426)
Across $44$ published-potential files ($43$ independent models), the oxide anchor holds for all
$32$ UIPs with no detectable generational difference ($p{=}0.68$, an honest null). The
...
% R3 LEVER 2026-07-16 (FINAL-SCORE-R3-2026-07-16.md, task item (a) / Reviewer 1 #1, #4):
% re-pitched for JCP's chemical-physics readership -- opens on the physical finding (oxide
% DFT reference-data self-consistency) rather than the CPU-only audit-protocol framing,
% which is demoted to a methods virtue in the paragraph below. All caveats (post-hoc,
% descriptive, non-transfer, cannot separate UIP error from label error) are unchanged and
% still stated here and in the Discussion; this changes order and emphasis only.
% R4 LEVER 2026-07-17 (FINAL-SCORE-R4-2026-07-17.md, single highest-leverage remaining lever
% / Reviewer 1 #1, Reviewer 2 #1): the physics-first opening stays, but its first two
% sentences asserted the trustworthiness reading close to fact ("exactly the chemistry
% where ... are least mutually self-consistent"; "is best understood as a signal ... is
% least trustworthy"). Hypothesis-marked to match SS IV(c)'s own standard ("cannot separate
% them" / "do not claim to have isolated a single physical cause"): "consistent with" and
% "one candidate explanation" replace the flat assertions. No claim or number changed.
...
\paragraph{Statistical-rigor and reproducibility precedent in the data-centric methodology
literature.} The audit-protocol framing here --- a frozen, provenance-gated re-analysis of
already-published prediction files rather than a new trained model --- has direct precedent in
the data-centric materials-ML methodology literature. Persaud et al.~\citep{persaud2024reproducibility}, in Digital Discovery, show that even a single published
materials-ML result can fail to reproduce without careful provenance tracking, the failure mode
our md5-gated, byte-identical reproduction is built to rule out. Ottomano
et al.~\citep{ottomano2024aggregation}, likewise in Digital Discovery, show that a common
materials-informatics assumption --- that aggregating more training data uniformly helps ---
does not survive a matched, controlled comparison, the same matched-yield discipline this paper
applies to abstention rather than to training-set size. Witman and Schindler's
{MatFold}~\citep{witman2025matfold}, also in Digital Discovery, standardizes cross-validation
splitting for materials-discovery models precisely because naive splits bias performance
estimates, echoing why this paper reports LOEO and WBM-round splits as robustness gates rather
than a single held-out number. On the confidence-signal side, a perspective in Digital Discovery
on uncertainty in ML atomistic modeling~\citep{grasselli2025uncertainty} surveys Bayesian
and ensembling UQ approaches broadly, and architecture-search-based ensembling for
molecular-property UQ~\citep{jiang2024autognnuq}, also in Digital Discovery, offers a richer
alternative confidence signal in the same spirit as the committee-variance comparison here
(Figure~\ref{fig:f5}); neither reports a chemistry-conditioned abstention value, the gap this
paper fills. Within \emph{this} journal specifically, the closest genuine anchor is Beck
et al.'s~\citep{beck2025} multi-head-committee uncertainty quantification for UIPs (discussed
above), which validates a richer confidence signal via force-error correlation and
active-learning data efficiency but --- like the Digital Discovery methodology literature above
--- does not test whether the value of any confidence signal for stability screening is itself
chemistry-conditioned.
...
% JCP requires an "Author declarations" section in the manuscript body (Conflict of
% Interest + Author Contributions), independent of and in addition to the portal-collected
% disclosures at submission (verified 2026-07-16, publishing.aip.org conflict-of-interest
% policy, effective Aug. 2021). Single-author paper: no conflicts, all contributions by AF.
\subsection*{Conflict of interest}
The author has no conflicts to disclose.

\subsection*{Use of AI tools}
AI-based coding and writing assistant tools (large language models) were used to assist with
drafting and editing of the manuscript text and analysis code; the author reviewed and
verified all content and takes full responsibility for it.
...
% JCP requires a Data availability statement per AIP Publishing's Research Data Policy
% (verified 2026-07-16, AIP data-availability-statement templates). Adapted from the
% canonical ../paper.tex "Data and code availability" section; content preserved.
...
each other's outputs; the present single R pipeline supersedes both.) The
...
DOI~\texttt{10.5281/zenodo.21130295}, consistent with AIP Publishing's Research Data Policy
template ``data openly available in a public repository that issues datasets with DOIs,''
alongside an actively maintained development mirror. The GitHub repository
is private during peer review (a portfolio-wide precaution against scooping) and will be made
public on acceptance; the Zenodo DOI is currently reserved on a draft deposition and becomes
...
genuinely deterministic and unaffected by the earlier hash staleness.
